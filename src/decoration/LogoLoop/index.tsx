...
const getResponsiveHorizontalGap = (gap: number, isVertical: boolean) => {
  if (isVertical || typeof window === 'undefined') return gap;
  if (window.innerWidth < 640) return Math.min(gap, 28);
  if (window.innerWidth < 1024) return Math.min(gap, 42);
  return gap;
};
...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    const [effectiveGap, setEffectiveGap] = useState<number>(gap);
    ...
    useEffect(() => {
      const updateGap = () => setEffectiveGap(getResponsiveHorizontalGap(gap, isVertical));
    ...
    useResizeObserver(updateDimensions, [containerRef, seqRef], [logos, effectiveGap, logoHeight, isVertical]);

    useImageLoader(seqRef, updateDimensions, [logos, effectiveGap, logoHeight, isVertical]);
    ...
    const cssVariables = useMemo(
      () =>
        ({
          '--logoloop-gap': `${effectiveGap}px`,
          '--logoloop-logoHeight': `${logoHeight}px`,
          ...(fadeOutColor && { '--logoloop-fadeColor': fadeOutColor })
        }) as React.CSSProperties,
      [effectiveGap, logoHeight, fadeOutColor]
    );
